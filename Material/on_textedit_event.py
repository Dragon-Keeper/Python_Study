...
class TextInput_No(TextInput):  # 这个类用于接收输入的变化然后赋值
    text_input = StringProperty()  # 将text_input定义为一个字符属性
    ...
    def _on_textedit(self, window, text):  # 这个是这个属性里的textedit（文件编辑）事件
        self.text_input = text  # 每次发生文件编辑事件时，将text的值赋值给text_input
...
class MainWidget(Widget):  # 这个类用于接收输入的赋值然后显示出来
    ...
    text2 = StringProperty()  # 将text定义为一个字符属性

    def __init__(self, **kwargs):
        super(MainWidget, self).__init__(**kwargs)
        self.text = ''  # 创建一个自定义事件,设定text的值为空，等待赋值
        self.text2 = ''  # 创建一个自定义事件,设定text的值为空，等待赋值
    ...
    def confim(self):  # 用来将输入的内容显示在label上，关联button的on_press
        ...
        self.text2 = self.ids["text_box2"].text  # 使用ids来访问带id标识的对象，将text_box赋值给text，而text_box取值自输入栏
        print(self.text)  # 成功显示text的值，一定要有self！
        print(self.text2)
    ...
    BoxLayout:
        ...
        BoxLayout:
            ...
            Label:
                ...
                text: "Enter text -> 2"
            TextInput:
                id: text_box2  # 设定id，用来给confim函数取值
                size_hint_x: 3
                focus: True
        BoxLayout:
            Button:
                size_hint_x: 2
                text: "Confirm text property  1"
            ...
            Label:
                size_hint_x: 3
                text: root.text2  # 从根层次取值text2，就是上面confim函数的作用
                canvas.before:
                    Color:
                        rgb: 0.5765 ,0.5765 ,0.5843
                    Rectangle:
